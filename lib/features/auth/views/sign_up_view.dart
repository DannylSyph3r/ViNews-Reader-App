...
class _UserSignUpViewState extends ConsumerState<UserSignUpView> {
  ...
  void updateSignupButton() {
    // Check if both the email and password text fields are not empty via the listeners and notifiers.
                    ...
                    children: [
                      ...
                      SizedBox(
                        ...
                        child: ViNewsAppTextFormField(
                            ...
                            suffixIconString: _emailSignUpAddressFieldController
                                        .text.isNotEmpty &&
                                    isEmailValid
                      ...
                      SizedBox(
                        ...
                        child: ViNewsAppTextFormField(
                          ...
                          validator: (value) {
                            ...
                            if (value != _passwordSignUpFieldController.text) {
                              return '⚠ Passwords do not match';
                            }
                            return null;
                          },
                          prefixIconString: ViNewsAppImagesPath.passwordIcon,
                          suffixIconString: confirmPasswordFieldObscured == true
                              ? ViNewsAppImagesPath.passwordShowIcon
                              : ViNewsAppImagesPath.passwordHideIcon,
                          onIconTap: () {
                            ref
                                .read(
                                    showSignUpConfirmPasswordProvider.notifier)
                                .update((state) => !state);
                            dropKeyboard();
                          },
                        ),
                      ...
                      ViNewsAppImageIconButton(
                        onButtonPress: () {
                          // Sign Up buttton press to handle 5 processs
                          dropKeyboard(); //1. Drop the keyboard
                          if (_signUpFormKey.currentState?.validate() == true) {
                            //2. Perform User Sign Up Process
                            ref.read(authNotifierProvider.notifier).userSignup(
                                  email: _emailSignUpAddressFieldController.text
                                      .trim(),
                                  password: _passwordSignUpFieldController.text
                                      .trim(),
                                  firstName:
                                      _firstNameSignUpController.text.trim(),
                                  lastName:
                                      _lastNameSignUpController.text.trim(),
                                );
                                ref.read(resendTimerProvider.notifier).startTimer(); //3. Start the timer for the Email Verification views "Resend Code" Button
                            resetButtonState(); //4. Reset the SIgn Up Button State
                          }
                        },
